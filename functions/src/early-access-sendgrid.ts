...
interface EarlyAccessSendgridData {
  email: string;
  name?: string;
  useCase?: string;
}
...
async function sendAdminNotification(submissionData: any) {
  if (!initializeSendGrid() || !process.env.ADMIN_NOTIFICATION_EMAIL) {
...
export const submitEarlyAccessSendgrid = onCall<EarlyAccessSendgridData>(async (request) => {
  try {
    // Get client IP for rate limiting
    const rawIP = request.rawRequest?.ip || request.rawRequest?.headers['x-forwarded-for'] || 'unknown';
    ...
    if (!existingSubmission.empty) {
      throw new HttpsError('already-exists', 'This email has already been submitted');
    }
    ...
    const submissionData = {
      ...
      source: "website",
      ipAddress: clientIP,
      userAgent: request.rawRequest?.headers['user-agent']?.substring(0, 500) || null,
      status: "pending"
    };
    ...
    Promise.all([
      ...
      sendAdminNotification({ ...submissionData, name })
    ]).catch(error => {
    ...
    throw new HttpsError('internal', 'An error occurred processing your request. Please try again.');
  }
});
